<div class="col-lg-5" style="padding: 35px;">
    <div class="content-box" id = "add_form_area">
        <?= form_open_multipart(site_url('home/sections/add/'.$course_id)), 'id = "ajaxFormForAdding" onsubmit="addSection(event)"' ?>
        <!--<form action="<?php //echo site_url('home/sections/add/'.$course_id); ?>" id = "ajaxFormForAdding" onsubmit="addSection(event)" method="post" enctype="multipart/form-data">-->
            ...
            <div class="content-update-box">
                <button type="submit" name = "submit" class="btn col-4"><?php echo $this->lang->line('add_section'); ?></button>
            </div>
        <?= form_close() ?>
    </div>
    <div class="content-box" id = "edit_form_area" style="display: none;">
        <?= form_open_multipart('', 'id = "ajaxFormForUpdating" onsubmit="updateSection(event)"') ?>
        <!--<form action="" id = "ajaxFormForUpdating" onsubmit="updateSection(event)" method="post" enctype="multipart/form-data">-->
            <div class="basic-group">
                <div class="form-group">
                    <label for="title"><?php echo $this->lang->line('title'); ?>:</label>
                    <input type="text" class="form-control" name = "title" id="section_title_for_editing" placeholder="<?php echo $this->lang->line('title'); ?>" required>
                </div>
            </div>
            <div class="content-update-box">
                <button type="submit" name = "submit" class="btn col-4"><?php echo $this->lang->line('update_section'); ?></button>
                <button type="button" name = "reset" class="btn col-4" onclick="resetForm()"><?php echo $this->lang->line('reset'); ?></button>
            </div>
        <?= form_close() ?>
            ...
            <tbody>
                <?php
                    ...
                    <?php foreach ($sections as $section):
                        ...
                        <td>
                            <a href="#" onclick="addNameAndIdOnForm('<?php echo $section ?>', '<?php echo $section_details['title'] ?>')"><?php echo $this->lang->line('edit'); ?></a>
                            <a href="#" onclick="confirm_modal('<?php echo site_url('home/sections/delete/'.$course_id.'/'.$section); ?>');"><?php echo $this->lang->line('delete'); ?></a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
